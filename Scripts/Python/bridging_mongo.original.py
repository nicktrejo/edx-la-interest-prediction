...
import storage.edxmongodbstore as MDB
import datetime
import sys
...
##############################################################
rootDir = Path(os.path.dirname(os.path.dirname(os.getcwd())))
            ...
            if str(userID) in str(general['id']) or sum(general['id'] == userID):
                ...
                if type(vCheck) != bool: 
                    if vCheck[0] == True: 
                        ...
                        """
                        elif vCheck[1] == 'enrollment':
                            #print(line)
                            element = re.search('"event": \"?\{(.*)\}\"?', line).group(1)

                            element = element.replace("\\", "")
                            #user_id = re.search('"user_id": \"([a-zA-Z0-9]*)\"', element).group(1) # already given by event-log information
                            enrMode = re.search('"mode": \"([a-zA-Z0-9]*)\"', element)
                            
                            #print(testCtr)
                            if checkValidity(enrMode, False) == True:

                                enrMode = enrMode.group(1)
                                    
                                enrollmentData.at[enrCtr, 'user_id'] = user_id
                                enrollmentData.at[enrCtr, 'mode'] = enrMode
                                addEnr = True
                        ...
                        if checkValidity(time, False) == True:
                            ...
                            if addEnr == True: 
                                ...
                                enrollmentData.at[enrCtr, 'time_day'] = time[0]
                                enrCtr = enrCtr + 1
                            """    
...
"""
new_columnNames = grades.columns.values
new_columnNames[0] = 'user_id'
grades.columns = new_columnNames
grades['user_id'] = grades.user_id.astype(str)

final_aggrData = pd.merge(final_aggrData, grades[['user_id','Grade','Certificate Eligible']], how='left', on = 'user_id')
"""
