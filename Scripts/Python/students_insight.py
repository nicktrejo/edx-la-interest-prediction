# -*- coding: utf-8 -*-
"""
Created on Thu Dec 04 19:58:57 2020
Based on briding_mongo.py by albbl

@author: nicktrejo
"""

# CAN EXECUTE THIS FILE FORM A PYTHON CONSOLE USING
# exec(open("/home/nicolas/Documents/MasterII/TFM/Proyecto/edx-la-interest-prediction/Scripts/Python/students_insight.py").read())

# CAN EXECUTE FROM LINUX TERMINAL USING
# python /home/nicolas/Documents/MasterII/TFM/Proyecto/edx-la-interest-prediction/Scripts/Python/students_insight.py
...
if __name__ == '__main__':
    ...
    if TREJO_PC:
        SELECTED_USERS = [23241503, 15312535,  # Certified
                          25492701, 23392051,  # Verified
                          332962, 11553882,  # Verified
                          ]

    # Basic blocks: video, problem, discussion, html
    ...
    print(f'Starting at: {time_lapse[-1]}')
    ...
    if total_routes == 0:
        ...
        raise Exception('No new routes to process')
    else:
        print(f'{total_routes} files to process')

    OUTPUT_DIR = ROOT_DIR / ('_OUTPUT' + '-' + last_date.strftime('%Y-%m-%d'))
    ...
    for index, row in routes.iterrows():
        ...
        with open(file_name, 'r') as f_events:
            ...
            for line_num, line in enumerate(f_events):
                ...
                sed_string = ''
                if TREJO_PC:
                    sed_string = sed_template.format(line_num+1, file_name)
                ...
                if TREJO_PC:
                    if int(user_id) not in SELECTED_USERS:
                        continue
                else:
    ...
    # columns = ['user_id', 'event_type', 'type_summary', 'dt_datetime', 'sed', 'video_id',
    ...
    cols_ordered = ['user_id', 'event_type', 'type_summary', 'dt_datetime',
                    'sed', 'video_id', 'video_code', 'grade', 'max_grade',
                    'attempts', 'module_id', 'enrollment_mode']
    df = df[cols_ordered]
    ...
    df.to_csv(OUTPUT_DIR / 'selectedUsers.csv', encoding='utf-8', index=False)
